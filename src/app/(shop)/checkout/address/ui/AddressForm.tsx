...
export const AddressForm = ({ countries, userStoreAddress = {} }: Props) => {
  ...
  //uso la session de nextauth para sacar id, el required es para devolverlo al login si no existe pero no deberia estar aqui...
  const {data:session} = useSession({
    required:true
  })

  const setAddess = useAddressStore((state) => state.setAddress);
  const addressStore = useAddressStore((state) => state.address);

  useEffect(() => {
    if(addressStore.firstName){
      reset(addressStore)//funcion de useForm que recibe valores si es que hay algo en el storage
    }
  },[addressStore]);

  const onSubmit = async (data: FormInputs) => {
    const {rememberAddress, ...restAddress} = data
    setAddess(restAddress);
